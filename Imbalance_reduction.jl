...
function optimize_imbalance(coalition, systemData)
    ...
    demand = systemData["demand_scenarios"]
    #demand = demand.+5/C
    T = min(TimeHorizon,size(systemData["price_prod_demand_df"])[1]) # Hours of forecast used
    S = length(demand[1,1,:]) # Number of scenarios, only one scenario for now
    ...
    @objective(model, Min, prob * sum((pos_imbal[t,s] + neg_imbal[t,s]) for t in 1:T for s in 1:S)) # Objective function: minimize total imbalance

    @constraint(model, [t = 1:T, s = 1:S],
                imbal[t,s] == sum(demand[t, c, s] for c in coalition_indexes)- prod[t] - bid[t]
    )
