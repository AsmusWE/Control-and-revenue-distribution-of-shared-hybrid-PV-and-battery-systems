...
function check_stability(payoffs, coalition_values, coalitions)
    ...
    instabilities = Dict()
    for c in coalitions
        if coalition_values[c] < sum(payoffs[i] for i in c) - 0.01 # Adding a small margin for floating point errors
    ...
    println("Maximum instability is for coalition ", max_instability_key, " with value ", max_instability, " corresponding to operating in the grand coalition being ", max_instability / coalition_values[max_instability_key] * 100, "% more expensive than operating in this coalition")
...
if !only_VCG
    ...
    decrease_in_cost_per_client = single_client_values .- vcg_values
    bar(clients, decrease_in_cost_per_client, xlabel="Client", ylabel="Decrease in Cost", title="Decrease in Cost per Client VCG")
